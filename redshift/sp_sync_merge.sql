CREATE OR REPLACE PROCEDURE etlfw_rs.sp_sync_merge_changes(batch_time timestamp without time zone, source_tbl character varying(256), target_tbl character varying(256), last_mod_col character varying(128), log_table character varying(256), businessdate_tbl character varying(256), max_rows bigint)
 LANGUAGE plpgsql
AS $$
DECLARE
    sql            VARCHAR(MAX) := '';
...
BEGIN
    ...
    IF pk_columns is null or pk_columns = '' THEN
        EXECUTE 'INSERT INTO '||log_table||' (batch_time, source_table, target_table, sync_column, sync_status, sync_queries, row_count)'||
                ' SELECT '''||batch_time||''','''||source_tbl||''','''||target_tbl||''','''||last_mod_col||
